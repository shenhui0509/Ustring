...
MemoryPool<T, block_size>::pad_pointer(data_pointer p,  size_type align) noexcept
{
...
{
  if(n == 1 && free_slot_ != nullptr){
    pointer result = reinterpret_cast<pointer>(free_slot_);
    free_slot_ = free_slot_->next;
    reinterpret_cast<slot_pointer>(result)->next = nullptr;
    return result;
  }
  if(cur_slot_ + n >= last_slot_){
    allocate_block();
  }

  /* Link the block memory into list*/
  /* Convinience for free*/
  slot_pointer p = cur_slot_;
  for(size_type i = 0; i < n - 1; i++){
    (p + i)->next = (p + i + 1);
  }
  (p + n)->next = nullptr;

  pointer result = reinterpret_cast<pointer>(cur_slot_);
  cur_slot_ += n;
  return result;
}
...
inline void  MemoryPool<T,block_size>::deallocate(pointer ptr, size_type n)
{
  ...
  slot_pointer head = reinterpret_cast<slot_pointer>(ptr);
  slot_pointer p = head;
  while(p->next)
    p = p->next;
